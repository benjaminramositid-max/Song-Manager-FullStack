...
async function fetchSongs() {
    try {
        ...
        showToast('Could not load songs from the server.', 'danger');
    }
}

const updateSongCount = () => {
    const countEl = document.getElementById('song-count');
    countEl.textContent = `${songsList.length} song${songsList.length !== 1 ? 's' : ''}`;
};
...
const renderSongs = () => {
    ...
    if (songsList.length === 0) {
        container.innerHTML = `
            <div class="text-center py-5 text-muted">
                ...
                <p class="mt-3">No songs yet. Add your first song!</p>
            </div>
        `;
        return;
    }
    ...
    container.innerHTML = songsList.map(song => {
        const genreClass = song.genre ? `genre-${song.genre.replace(/\s+/g, '')}` : 'genre-Other';
                    ...
                    <div class="d-flex justify-content-between align-items-start">
                        <div class="flex-grow-1">
                            ...
                            <small class="text-muted d-block mt-2">
                                ID: ${song.id} | Added: ${formatDate(song.created_at)}
                            </small>
                        </div>
...
window.deleteSong = async (id) => {
    ...
    if (!confirm(`Are you sure you want to remove "${song?.title || 'this song'}" from your playlist?`)) {
        return;
    }
...
const editSongModalElement = document.getElementById('editSongModal');
editSongModal = new bootstrap.Modal(editSongModalElement);
